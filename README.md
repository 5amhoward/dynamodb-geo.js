...
This project is forked from rh389/dynamodb-geo to add support for GSIs and include date/time in the hash. rh389/dynamodb-geo is an unofficial port of [awslabs/dynamodb-geo][dynamodb-geo], bringing creation and querying of geospatial data to Node JS developers using [Amazon DynamoDB][dynamodb].

## Features

- **Box Queries:** Return all of the items marked as that fall within a pair of geo points that define a rectangle as projected onto a sphere AND that are marked with a time window that contains a defined query time window.
- **Radius Queries:** Return all of the items that are within a given radius of a geo point AND that are marked with a time window that contains a defined query time window.
- **Basic CRUD Operations:** Create, retrieve, update, and delete geospatial time-series data items.
...
Using [npm] or [yarn]:
`npm install --save dynamodb-geo-time` or `yarn add dynamodb-geo-time`.
...
The year and week number are included in the hash key. Queries must include a time window object with a Date Object for start and end. If the time window for a geopoint spans multiple weeks then a record will be created for each week.
...
myGeoTableManager
  .putPoint({
    ...
    HashKeyValue: {S: 'abcde'}, // Use this for the id of the object or a value that will be used to identify/query/update the record.
    GeoPoint: {
    ...
    TimeWindow: {
      // An object specifying the start and end of a time window as Date objects. Used to build the hashkey and included within the record.
      from: new Date('2019-06-27T21:44:09.129Z'),
      to: new Date('2019-06-28T21:44:09.129Z')
    }
    PutItemInput: {
...
Note that you cannot update the hash key, range key, geohash, timeWindow or geoJson. If you want to change these, you'll need to recreate the record.

You must specify a `RangeKeyValue`, a `HashKeyValue`, and an `UpdateItemInput` matching the [DynamoDB UpdateItem][updateitem] request (`TableName` and `Key` are filled in for you).

```js
myGeoTableManager
  .updatePoint({
    RangeKeyValue: { S: '1234' },
    HashKeyValue: { S: 'abcde' },
    UpdateItemInput: {
...
You must specify a `RangeKeyValue` and a `HashKeyValue`. Optionally, you can pass `DeleteItemInput` matching [DynamoDB DeleteItem][deleteitem] request (`TableName` and `Key` are filled in for you).
...
The library now supports composite keys. The Hash Key and Range Key used for the index is customisable.
